...
- Googleログイン認証
- 支出の登録・編集・削除（CRUD）
- カテゴリ・決済方法・引き落とし方法のマスターデータ管理（カスタム追加・削除）
- 月ごとの集計グラフ（カテゴリ別・引き落とし方法別）
- 引き落とし方法別の明細一覧（クレカ明細との照らし合わせ用）
- 月の切り替え機能
- CSVエクスポート
- 多言語対応（日本語・英語・タガログ語）
- スマホUI最適化
...
- **マスターデータと実績データの分離設計**
  カテゴリ等を変更しても過去のログには影響しない設計
- **FirebaseのコールバックはAngularのZone外で実行される**ため、`ChangeDetectorRef.detectChanges()` で変更検知を手動でトリガー
- **引き落とし方法別の集計ビュー**でクレカ明細との照らし合わせを効率化
- Firestoreのサブコレクション設計（`users/{uid}/entries`）でユーザーごとのデータを完全分離
